    ...
    for (iInteg = 0; iInteg < theRule->n; iInteg++) {
      ...
      for (i = 0; i < theSpace->n; i++) {
        for (j = 0; j < theSpace->n; j++) {
          A[mapX[i]][mapX[j]] +=
              (dphidx[i] * a * dphidx[j] + dphidy[i] * c * dphidy[j]) * jac *
              weight;
          A[mapX[i]][mapY[j]] +=
              (dphidx[i] * b * dphidy[j] + dphidy[i] * c * dphidx[j]) * jac *
              weight;
          A[mapY[i]][mapX[j]] +=
              (dphidy[i] * b * dphidx[j] + dphidx[i] * c * dphidy[j]) * jac *
              weight;
          A[mapY[i]][mapY[j]] +=
              (dphidy[i] * a * dphidy[j] + dphidx[i] * c * dphidx[j]) * jac *
              weight;
        }
      }
      for (i = 0; i < theSpace->n; i++) {
    ...
    if (type == NEUMANN_Y) {
      for (iElem = 0; iElem < theDomain->nElem; iElem++) {
        ...
        for (iInteg = 0; iInteg < theRule->n; iInteg++) {
          ...
          double jac = sqrt((x[0] - x[1]) * (x[0] - x[1]) +
                            (y[0] - y[1]) * (y[0] - y[1])) /
                       2.0;
...
void femElasticityAssembleElementsCsr(femProblemCsr *theProblem) {
  ...
  double rho = theProblem->rho;
  double g = theProblem->g;
  double *data = theSystem->data;
  int *row_pointer = theSystem->row_pointer;
  int *column = theSystem->column;
  double *B = theSystem->B;
  ...
  for (iElem = 0; iElem < theMesh->nElem; iElem++) {
    ...
    for (iInteg = 0; iInteg < theRule->n; iInteg++) {
      ...
      for (i = 0; i < theSpace->n; i++) {
        for (j = 0; j < theSpace->n; j++) {
          for (int k = row_pointer[mapX[i]]; k < row_pointer[mapX[i] + 1]; k++) {
            if (column[k] == mapX[j]) {
              data[k] +=
                  (dphidx[i] * a * dphidx[j] + dphidy[i] * c * dphidy[j]) *
                  jac * weight;
            }
            if (column[k] == mapY[j]) {
              data[k] +=
                  (dphidy[i] * a * dphidy[j] + dphidx[i] * c * dphidx[j]) *
                  jac * weight;
            }
          }
          for (int k = row_pointer[mapY[i]]; k < row_pointer[mapY[i] + 1];
               k++) {
            if (column[k] == mapX[j]) {
              data[k] +=
                  (dphidy[i] * b * dphidx[j] + dphidx[i] * c * dphidy[j]) *
                  jac * weight;
            }
            if (column[k] == mapY[j]) {
              data[k] +=
